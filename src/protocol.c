...
int16_t mems_read_serial(mems_info* info, uint8_t* buffer, uint16_t quantity)
{
    ...
    if (mems_is_connected(info)) {
        do {
            ...
            if ((ReadFile(info->sd, (UCHAR*)buffer_pt, quantity, &w32BytesRead, NULL) == TRUE) && (w32BytesRead > 0)) {
...
bool mems_read_iac_position(mems_info* info, uint8_t* position)
{
    ...
    if (mems_lock(info)) {
        status = mems_send_command(info, MEMS_GetIACPosition) && (mems_read_serial(info, position, 1) == 1);
        mems_unlock(info);
    }
    return status;
}
...
bool mems_move_iac(mems_info* info, uint8_t desired_pos)
{
    ...
    if (mems_read_iac_position(info, &current_pos)) {
        if ((desired_pos < current_pos) || ((desired_pos > current_pos) && (current_pos < IAC_MAXIMUM))) {
...
bool mems_test_actuator(mems_info* info, actuator_cmd cmd, uint8_t* data)
{
    ...
    if (mems_lock(info)) {
        if (mems_send_command(info, cmd) && (mems_read_serial(info, &response, 1) == 1)) {
            if (data) {
                *data = response;
            }
            status = true;
...
bool mems_clear_faults(mems_info* info)
{
    ...
    if (mems_lock(info)) {
        ...
        status = mems_send_command(info, (uint8_t)MEMS_ClearFaults) && (mems_read_serial(info, &response, 1) == 1);
        {
            status = true;
        }
        mems_unlock(info);
...
bool mems_heartbeat(mems_info* info)
{
    ...
    if (mems_lock(info)) {
        // send the command and check for one additional byte after the
        // echoed command byte (should be 0x00)
        if (mems_send_command(info, (uint8_t)MEMS_Heartbeat) && (mems_read_serial(info, &response, 1) == 1)) {
            status = true;
        }
        mems_unlock(info);
    }
